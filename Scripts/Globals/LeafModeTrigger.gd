...
const final_tween_light_energy = 1.5
const final_tween_subtract_light = 0.885

func _on_body_entered(body: Node2D):
	if not body.is_in_group("Player"): return
	Player.time_spend_not_walking = 0.0
	Player.tween_value(0.7)
	LeafMode.tween_subtract_light(final_tween_subtract_light)
	Player.tween_color(Color.LIGHT_GREEN)
	Player.tween_light_energy(final_tween_light_energy)
...
func _on_body_exited(body: Node2D) -> void:
	...
	LeafMode.tween_subtract_light(0)
